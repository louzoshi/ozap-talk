...
namespace OzapTalk.ArchitectureTests;
...
public class ModuleBoundaryTests
{
    ...
    private static Assembly Load(string name) => Assembly.Load($"OzapTalk.{name}");
    ...
    public void A_module_does_not_reference_another_modules_internals()
    {
        foreach (var module in Modules)
        {
            var otherModuleNamespaces = Modules
                .Where(m => m != module)
                .SelectMany(m => new[] { $"OzapTalk.{m}.Domain", $"OzapTalk.{m}.Infrastructure" })
                .ToArray();
            ...
            foreach (var layer in new[] { "Domain", "Application", "Infrastructure", "Api" })
            {
                ...
                result.IsSuccessful.Should().BeTrue(
                    $"OzapTalk.{module}.{layer} reached into another module: {string.Join(", ", result.FailingTypeNames ?? [])}");
            }
        }
    }
}
